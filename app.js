...
const TRANSLATIONS = {
  id: {
    ...
    monthlyModeDescription: 'Rekap data gempa dari file Excel yang dapat diperbarui otomatis.',
    ...
    monthlyDashboardTitle: 'Rekap Gempa Bulanan',
    sourceRealtime: 'REAL-TIME BMKG',
    sourceMonthly: 'REKAP BULANAN',
    monthlyStatus: 'Rekap bulanan • {count} kejadian',
    ...
    footerMonthlySource: '© 2026 QuakePulse. Rekap data gempa bulanan.',
    footerMonthlyDeveloper: 'Rekap bulanan dibuat otomatis dari file Excel.',
    navMap: 'Peta',
  ...
  en: {
    ...
    monthlyModeDescription: 'Earthquake recap generated automatically from an Excel file.',
    ...
    monthlyDashboardTitle: 'Monthly Earthquake Recap',
    sourceRealtime: 'REAL-TIME BMKG',
    sourceMonthly: 'MONTHLY RECAP',
    monthlyStatus: 'Monthly recap • {count} events',
    ...
    footerMonthlySource: '© 2026 QuakePulse. Monthly earthquake recap.',
    footerMonthlyDeveloper: 'The monthly recap is generated automatically from an Excel file.',
    navMap: 'Map',
...
function monthlyMetadataText() {
  const metadata = state.monthlyMetadata || (state.mode === 'monthly' ? state.metadata : null) || {};
  const isEnglish = state.language === 'en';
  const count = Number(metadata.record_count || metadata.total_events || 0);
  const period = isEnglish
    ? (metadata.period_label_en || metadata.period_label_id || '')
    : (metadata.period_label_id || metadata.period_label_en || '');
  const periodShort = isEnglish
    ? (metadata.period_short_en || metadata.period_label_en || '')
    : (metadata.period_short_id || metadata.period_label_id || '');
  const sourceFile = metadata.source_file || (isEnglish ? 'Excel file' : 'file Excel');
  const countText = count ? formatNumber(count) : '';

  if (isEnglish) {
    return {
      description: count && period
        ? `Recap of ${countText} earthquake events during ${period}.`
        : t('monthlyModeDescription'),
      dashboardTitle: period ? `${period} Earthquake Recap` : t('monthlyDashboardTitle'),
      sourceBadge: periodShort ? `${periodShort} RECAP` : t('sourceMonthly'),
      status: period
        ? `${period} monthly recap • ${countText || formatNumber(state.pointsData?.features?.length || 0)} events`
        : t('monthlyStatus', { count: countText || formatNumber(state.pointsData?.features?.length || 0) }),
      footerSource: period
        ? `© ${new Date().getFullYear()} QuakePulse. ${period} earthquake recap.`
        : t('footerMonthlySource'),
      footerDeveloper: `Monthly recap generated automatically from ${sourceFile}.`
    };
  }

  return {
    description: count && period
      ? `Rekap ${countText} kejadian gempa pada periode ${period}.`
      : t('monthlyModeDescription'),
    dashboardTitle: period ? `Rekap Gempa ${period}` : t('monthlyDashboardTitle'),
    sourceBadge: periodShort ? `REKAP ${periodShort}` : t('sourceMonthly'),
    status: period
      ? `Rekap bulanan ${period} • ${countText || formatNumber(state.pointsData?.features?.length || 0)} kejadian`
      : t('monthlyStatus', { count: countText || formatNumber(state.pointsData?.features?.length || 0) }),
    footerSource: period
      ? `© ${new Date().getFullYear()} QuakePulse. Rekap data gempa ${period}.`
      : t('footerMonthlySource'),
    footerDeveloper: `Rekap bulanan dibuat otomatis dari ${sourceFile}.`
  };
}

function updateMonthlyChooserText() {
  const text = monthlyMetadataText();
  document.querySelectorAll('[data-i18n="monthlyModeDescription"]').forEach((element) => {
    element.textContent = text.description;
  });
}
...
function updateLiveStatus(status = 'live') {
  ...
  if (status === 'monthly') {
    elements.liveStatus.textContent = monthlyMetadataText().status;
    return;
  }
  if (status === 'fallback') {
...
function updateModeUI() {
  ...
  const monthlyText = monthlyMetadataText();
  ...
  elements.dashboardTitle.textContent = isRealtime ? t('realtimeDashboardTitle') : monthlyText.dashboardTitle;
  elements.sourceBadgeIcon.textContent = isRealtime ? 'sensors' : 'calendar_month';
  elements.sourceBadgeText.textContent = isRealtime ? t('sourceRealtime') : monthlyText.sourceBadge;
  elements.footerSource.textContent = isRealtime ? t('footerRealtimeSource') : monthlyText.footerSource;
  elements.footerDeveloper.textContent = isRealtime ? t('footerRealtimeDeveloper') : monthlyText.footerDeveloper;
}
...
async function loadMonthlyMode({ initial = true } = {}) {
  ...
  state.pointsData = loaded.pointsData;
  state.metadata = loaded.metadata;
  state.monthlyMetadata = loaded.metadata;
  updateMonthlyChooserText();
  updateModeUI();
  normalizeData();
...
async function initialize() {
  try {
    ...
    try {
      state.monthlyMetadata = await fetchJson(FALLBACK_DATA_PATHS.metadata);
    } catch (metadataError) {
      console.warn('Monthly metadata preload failed:', metadataError);
    }
